...
<?php
        ...
        ?>
            ...
            <div data-ns="css.sw_push"<?php $visible('css.sw_push'); ?>>
                ...
                <label><input type="checkbox" value="1" name="o10n[css.sw_push.filter.enabled]" data-json-ns="1"<?php $checked('css.sw_push.filter.enabled'); ?> /> Enable filter</label>
                <span data-ns="css.sw_push.filter"<?php $visible('css.sw_push.filter'); ?>>
                    <select name="o10n[css.sw_push.filter.type]" data-ns-change="css.sw_push.filter" data-json-default="<?php print esc_attr(json_encode('include')); ?>">
                        <option value="include"<?php $selected('css.sw_push.filter.type', 'include'); ?>>Include List</option>
                        <option value="exclude"<?php $selected('css.sw_push.filter.type', 'exclude'); ?>>Exclude List</option>
                    </select>
                </span>
            </div>
<?php
...
?>
    ...
    <tr valign="top" data-ns="css.sw_push.filter"<?php $visible('css.sw_push.filter', ($get('css.sw_push.filter.type') === 'include')); ?> data-ns-condition="css.sw_push.filter.type==include">
        <th scope="row">&nbsp;</th>
        <td style="padding-top:0px;">
            <h5 class="h">&nbsp;Service Worker Push Include List</h5>
            <textarea class="json-array-lines" name="o10n[css.sw_push.filter.include]" data-json-type="json-array-lines" placeholder="Leave blank to push all stylesheets..."><?php $line_array('css.sw_push.filter.include'); ?></textarea>
            <p class="description">Enter (parts of) stylesheet <code>&lt;link&gt;</code> elements to push, e.g. <code>bootstrap.min.css</code> or <code>id="stylesheet"</code>. One match string per line.</p>
        </td>
    </tr>
    <tr valign="top" data-ns="css.sw_push.filter"<?php $visible('css.sw_push.filter', ($get('css.sw_push.filter.type') === 'exclude')); ?> data-ns-condition="css.sw_push.filter.type==exclude">
        <th scope="row">&nbsp;</th>
        <td style="padding-top:0px;">
            <h5 class="h">&nbsp;Service Worker Push Exclude List</h5>
            <textarea class="json-array-lines" name="o10n[css.sw_push.filter.exclude]" data-json-type="json-array-lines"><?php $line_array('css.sw_push.filter.exclude'); ?></textarea>
            <p class="description">Enter (parts of) stylesheet <code>&lt;link&gt;</code> elements to exclude from being pushed. One match string per line.</p>
        </td>
    </tr>
    <tr valign="top" data-ns="css.async"<?php $visible('css.async'); ?>>
